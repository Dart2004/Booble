<hr>
<hr>
<hr>
<div style="text-align:center">
<h1><form action="card.php" method='post'>
<button name="name" type="submit" value="Nat#4853" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
$barr=$_POST["barr"];
$forr=$_POST["forr"];
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("Nat#4853Level");
if($hosp=="1"){
  $H=file_get_contents("Nat#4853deadsold.txt");
  $W=file_get_contents("Nat#4853Wood");
  $S=file_get_contents("Nat#4853Stone");
  $F=file_get_contents("Nat#4853Fish");
  $L=file_get_contents("Nat#4853Level");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){

  }
  else{
    $h=@fopen("Nat#4853deadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("Nat#4853Wood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("Nat#4853sold.txt");
    $h=@fopen("Nat#4853sold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("Nat#4853Stone","w");
    fwrite($w,$S-10);
    $w=@fopen("Nat#4853Fish","w");
    fwrite($w,$F-10);
  }
}
if($barr==1){
  $W=file_get_contents("Nat#4853Wood");
  $S=file_get_contents("Nat#4853Stone");
  $F=file_get_contents("Nat#4853Fish");
  if($W-10<0||$S-10<0||$F-10<0){

  }
  else{
    $H=file_get_contents("Nat#4853sold.txt");
    $h=@fopen("Nat#4853sold.txt","w");
    fwrite($h,$H+1*$L);
    $w=@fopen("Nat#4853Wood","w");
    fwrite($w,$W-10);
    $w=@fopen("Nat#4853Stone","w");
    fwrite($w,$S-10);
    $w=@fopen("Nat#4853Fish","w");
    fwrite($w,$F-10);
  }
}
if($forr==1){
  $H=file_get_contents("Nat#4853Wood");
  $h=@fopen("Nat#4853Wood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("Nat#4853Fish");
  $h=@fopen("Nat#4853Fish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("Nat#4853Stone");
  $h=@fopen("Nat#4853Stone","w");
  fwrite($h,$H+9*$L);
}
?>
<table>
<td style="width:calc(100% - 50px)">
<?php
  include "Nat#4853Build";
?>
</td>
<td style="text-align:right;width:50px">
<?php
  $W=file_get_contents("Nat#4853Wood");
  $S=file_get_contents("Nat#4853Stone");
  $F=file_get_contents("Nat#4853Fish");
  include "Wood";
  echo $W;
  echo "<br>";
  include "Ston";
  echo $S;
  echo "<br>";
  include "Fish";
  echo $F;
if($fish==2){
  $W=file_get_contents("Nat#4853Wood");
  $S=file_get_contents("Nat#4853Stone");
  $F=file_get_contents("Nat#4853Fish");
  if($W-1000<0||$S-1000<0){

  }
  else{
  $w=@fopen("Nat#4853Wood","w");
  fwrite($w,$W-1000);
  $s=@fopen("Nat#4853Stone","w");
  fwrite($s,$S-1000);
  $f=@fopen("Nat#4853Fish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("Nat#4853Level");
  $lev=@fopen("Nat#4853Level","w");
  fwrite($lev,$level+1/3);
  $build=file_get_contents("Nat#4853Build");
  $building=@fopen("Nat#4853Build","w");
  fwrite($building,$build.'<td><form action="Nat#4853Menu.php" method="post"><button name="fish" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}

if($forr==2){
  $W=file_get_contents("Nat#4853Wood");
  $S=file_get_contents("Nat#4853Stone");
  $F=file_get_contents("Nat#4853Fish");
  if($F-1000<0||$S-1000<0){

  }
  else{
  $w=@fopen("Nat#4853Wood","w");
  fwrite($w,$W-1000);
  $s=@fopen("Nat#4853Stone","w");
  fwrite($s,$S-1000);
  $f=@fopen("Nat#4853Fish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("Nat#4853Level");
  $lev=@fopen("Nat#4853Level");
  fwrite($lev,$level+1/3,"w");
  $build=file_get_contents("Nat#4853Build");
  $building=@fopen("Nat#4853Build","w");
  fwrite($building,$build.'<td><form action="Nat#4853Menu.php" method="post"><button name="forr" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}

if($ston==2){
  $W=file_get_contents("Nat#4853Wood");
  $S=file_get_contents("Nat#4853Stone");
  $F=file_get_contents("Nat#4853Fish");
  if($W-1000<0||$F-1000<0){

  }
  else{
  $w=@fopen("Nat#4853Wood","w");
  fwrite($w,$W-1000);
  $s=@fopen("Nat#4853Stone","w");
  fwrite($s,$S-1000);
  $f=@fopen("Nat#4853Fish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("Nat#4853Level");
  $lev=@fopen("Nat#4853Level","w");
  fwrite($lev,$level+1/3);
  $build=file_get_contents("Nat#4853Build");
  $building=@fopen("Nat#4853Build","w");
  fwrite($building,$build.'<td><form action="Nat#4853Menu.php" method="post"><button name="ston" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}

?>
</td>
</tr>
<tr style="text-align:center">
<h1>Building</h1>
</tr><tr>
<td><form action="" method="post">
<button name="ston" value="2">Add Mountain</button>
</form><td>
<td><form action="" method="post">
<button name="forr" value="2">Add Forrest</button>
</form><td>
<td><form action="" method="post">
<button name="fish" value="2">Add Bay</button>
</form><td>