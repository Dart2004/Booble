<body style="background-color:black">
<hr>
<hr>
<hr>
<div style="text-align:center">
<h1><form action="card.php" method='post'>
<button style="border-radius:10px"  name="name" type="submit" value="TiLu" style="border-radius:5">Map</button><div style="color:white">You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
$barr=$_POST["barr"];
$forr=$_POST["forr"];
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("TiLuLevel");
if($hosp=="1"){
  $H=file_get_contents("TiLudeadsold.txt");
  $W=file_get_contents("TiLuWood");
  $S=file_get_contents("TiLuStone");
  $F=file_get_contents("TiLuFish");
  $L=file_get_contents("TiLuLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){

  }
  else{
    $h=@fopen("TiLudeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("TiLuWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("TiLusold.txt");
    $h=@fopen("TiLusold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("TiLuStone","w");
    fwrite($w,$S-10);
    $w=@fopen("TiLuFish","w");
    fwrite($w,$F-10);
  }
}
if($barr==1){
  $W=file_get_contents("TiLuWood");
  $S=file_get_contents("TiLuStone");
  $F=file_get_contents("TiLuFish");
  if($W-10<0||$S-10<0||$F-10<0){

  }
  else{
    $H=file_get_contents("TiLusold.txt");
    $h=@fopen("TiLusold.txt","w");
    fwrite($h,$H+1*$L);
    $w=@fopen("TiLuWood","w");
    fwrite($w,$W-10);
    $w=@fopen("TiLuStone","w");
    fwrite($w,$S-10);
    $w=@fopen("TiLuFish","w");
    fwrite($w,$F-10);
  }
}
if($forr==1){
  $H=file_get_contents("TiLuWood");
  $h=@fopen("TiLuWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("TiLuFish");
  $h=@fopen("TiLuFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("TiLuStone");
  $h=@fopen("TiLuStone","w");
  fwrite($h,$H+9*$L);
}
?>
<table>
<td style="width:calc(100% - 50px)">
<?php
  include "TiLuBuild";
?>
</td>
<td style="text-align:right;width:50px">
<?php
  $W=file_get_contents("TiLuWood");
  $S=file_get_contents("TiLuStone");
  $F=file_get_contents("TiLuFish");
  include "Wood";
  echo "<div style='color:white'>".$W;
  echo "<br>";
  include "Ston";
  echo "<div style='color:white'>".$S;
  echo "<br>";
  include "<font color='white'>Fish</font>";
  echo "<div style='color:white'>".$F;
if($fish==2){
  $W=file_get_contents("TiLuWood");
  $S=file_get_contents("TiLuStone");
  $F=file_get_contents("TiLuFish");
  if($W-1000<0||$S-1000<0){

  }
  else{
  $w=@fopen("TiLuWood","w");
  fwrite($w,$W-1000);
  $s=@fopen("TiLuStone","w");
  fwrite($s,$S-1000);
  $f=@fopen("TiLuFish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("TiLuLevel");
  $lev=@fopen("TiLuLevel","w");
  fwrite($lev,$level+1/3);
  $build=file_get_contents("TiLuBuild");
  $building=@fopen("TiLuBuild","w");
  fwrite($building,$build.'<td><form action="TiLuMenu.php" method="post"><button style="border-radius:10px"  name="fish" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}

if($forr==2){
  $W=file_get_contents("TiLuWood");
  $S=file_get_contents("TiLuStone");
  $F=file_get_contents("TiLuFish");
  if($F-1000<0||$S-1000<0){

  }
  else{
  $w=@fopen("TiLuWood","w");
  fwrite($w,$W-1000);
  $s=@fopen("TiLuStone","w");
  fwrite($s,$S-1000);
  $f=@fopen("TiLuFish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("TiLuLevel");
  $lev=@fopen("TiLuLevel");
  fwrite($lev,$level+1/3,"w");
  $build=file_get_contents("TiLuBuild");
  $building=@fopen("TiLuBuild","w");
  fwrite($building,$build.'<td><form action="TiLuMenu.php" method="post"><button style="border-radius:10px"  name="forr" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}

if($ston==2){
  $W=file_get_contents("TiLuWood");
  $S=file_get_contents("TiLuStone");
  $F=file_get_contents("TiLuFish");
  if($W-1000<0||$F-1000<0){

  }
  else{
  $w=@fopen("TiLuWood","w");
  fwrite($w,$W-1000);
  $s=@fopen("TiLuStone","w");
  fwrite($s,$S-1000);
  $f=@fopen("TiLuFish","w");
  fwrite($f,$F-1000);
  $level=file_get_contents("TiLuLevel");
  $lev=@fopen("TiLuLevel","w");
  fwrite($lev,$level+1/3);
  $build=file_get_contents("TiLuBuild");
  $building=@fopen("TiLuBuild","w");
  fwrite($building,$build.'<td><form action="TiLuMenu.php" method="post"><button style="border-radius:10px"  name="ston" value=1 type="submit" style="background-color:white;border:none"><img style="width:40px;height:40px" src="data:image/png;base64,"></button></form></td>');
  }
}

?>
</td>
</tr>
<tr style="text-align:center">
<h1 style="color:white">Building</h1>
</tr><tr>
<td><form action="" method="post">
<button style="border-radius:10px"  name="ston" value="2">Add Mountain</button>
</form><td>
<td><form action="" method="post">
<button style="border-radius:10px"  name="forr" value="2">Add Forrest</button>
</form><td>
<td><form action="" method="post">
<button style="border-radius:10px"  name="fish" value="2">Add Bay</button>
</form><td>