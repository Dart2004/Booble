...
<button name="name" type="submit" value="fishie" style="border-radius:5">Map</button>You</h1></form>
<hr>
</div>
<?php
$hosp=$_POST["hosp"];
...
$fish=$_POST["fish"];
$ston=$_POST["ston"];
$L=file_get_contents("fishieLevel");
if($hosp=="1"){
  $H=file_get_contents("fishiedeadsold.txt");
  $W=file_get_contents("fishieWood");
  $S=file_get_contents("fishieStone");
  $F=file_get_contents("fishieFish");
  $L=file_get_contents("fishieLevel");
  if($H-10<0||$W-10<0||$S-10<0||$F-10<0){

  }
  else{
    $h=@fopen("fishiedeadsold.txt","w");
    fwrite($h,$H-10);
    $w=@fopen("fishieWood","w");
    fwrite($w,$W-10);
    $H=file_get_contents("fishiesold.txt");
    $h=@fopen("fishiesold.txt","w");
    fwrite($h,$H+10*$L);
    $w=@fopen("fishieStone","w");
    fwrite($w,$S-10);
    $w=@fopen("fishieFish","w");
    fwrite($w,$F-10);
  }
}
if($barr==1){
...
if($forr==1){
  $H=file_get_contents("fishieWood");
  $h=@fopen("fishieWood","w");
  fwrite($h,$H+9*$L);
}
if($fish==1){
  $H=file_get_contents("fishieFish");
  $h=@fopen("fishieFish","w");
  fwrite($h,$H+9*$L);
}
if($ston==1){
  $H=file_get_contents("fishieStone");
  $h=@fopen("fishieStone","w");
  fwrite($h,$H+9*$L);
}
?>
<table>
